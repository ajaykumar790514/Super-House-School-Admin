...
<input type="hidden" value="<?php echo $parent_cat_id; ?>" id="parent_cat_id">
    ...
    if(confirm('If you are applying offer on category it will replace offers existing on products.') == true)
        {
            ...
            $.ajax({
                url: "<?php echo base_url('coupons-offers/add_offer_cat'); ?>",
                method: "POST",
                data: {
                    oid:oid,
...
<script type="text/javascript">
   function remove_offer_cat(oid,value,discount_type)
   {
    if(confirm('Are you sure?') == true)
    {
        ...
        var shop_id = $('#shop_id').val();
        $.ajax({
            url: "<?php echo base_url('coupons-offers/remove_offer_cat'); ?>",
            method: "POST",
            data: {
                oid:oid,
                parent_cat_id:parent_cat_id,
