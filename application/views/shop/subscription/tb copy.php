...
<div class="row">
        ...
        <div class="col-2">
            <div class="form-group">
                ...
                <input type="date" class="form-control" name="to_date" id="to_date" value="<?php if(!empty($to_date)){echo $to_date; }?>" onchange="filter_subscription(this.value)">
            </div>
        </div>
        <div class="col-2">
            <div class="form-group">
                <label class="control-label">Plan type:</label>
                <select class="form-control" style="width:100%;" name="plan_type_id" id="plan_type" onchange="filter_by_plan_type(this.value)">
        ...
        <div class="col-2">
            <div class="form-group">
                ...
                <select class="form-control" style="width:100%;" name="time_slot_id" id="delivery_slot" onchange="filter_by_delivery_slot(this.value)">
                <option value="">Select</option>
        ...
        <div class="col-2">
            <div class="form-group">
                ...
                <select class="form-control" style="width:100%;" name="user_id" id="user_id" onchange="filter_by_customer(this.value)">
                <option value="">Select</option>
        ...
        <div class="col-2">
            <div class="form-group">
                ...
                <select class="form-control" style="width:100%;" name="address_id" id="address_id" onchange="filter_by_pincode(this.value)">
                <option value="">Select</option>
                <?php if(!empty($to_date)){
                    $pincodes = array();
                    foreach($subscriptions as $subscr){ 
...
<?php if(!empty($to_date)) { ?>
    ...
    <div class="col-md-2">
        <a href="<?= base_url('Shop_subscription/subscription_data/export_to_excel/'.$to_date.'/'.$plan_type_id.'/'.$time_slot_id.'/'.$user_id.'/'.$product_id."/".$status_id."/".$address_id); ?>" class="btn btn-primary btn-sm mb-3"><i class="fas fa-arrow-down"></i> Export to Excel</a>
    </div>
    <div id="grid_table" class="table-responsive">
        <table class="jsgrid-table">
            ...
            <?php $i=$page;$flag='0'; foreach($subscriptions as $subscription){ 
                foreach($subscription as $value){$flag='0';
            ...
            <tr class="jsgrid-filter-row">
                ...
                <td class="jsgrid-cell jsgrid-align-center"><?= $value->house_no.','.$value->address.','.$value->pincode; ?> </td>
                ...
                <td class="jsgrid-cell jsgrid-align-center">
                    ...
                    <?php } else {?>
                        <button class="btn btn-sm btn-primary order_button<?=$value->sid ?>" onclick="create_subscription_order(<?=$value->sid;?>)">Generate order</button>
                    <?php }?>
...
<script type="text/javascript">
   function filter_subscription(to_date)
   {
    ...
    $.ajax({
        url: "<?php echo base_url('Shop_subscription/subscription_data/tb'); ?>",
        method: "POST",
        data: {
            to_date:to_date,
   ...
   function create_subscription_order(sid)
   {
    if(confirm('Are you sure?') == true)
    {
        $.ajax({
            url: "<?php echo base_url('Shop_subscription/create_subscription_order'); ?>",
            method: "POST",
            data: { sid:sid
            },
            success: function(data){
       ...
       function update_order_status(sid)
        {
            if(confirm('Are you sure?') == true)
            {
                var status_id = $("#order-status option:selected").val();
                $.ajax({
                    url: "<?php echo base_url('Shop_subscription/subscription_data/update_order_status'); ?>",
                    method: "POST",
                    data: { 
                        sid:sid,
                        status_id:status_id
